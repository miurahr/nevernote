...
private char[] washAlphabetIntoReplaceList(char[] alphabet) {
    ...
    for (char element : alphabet) {
      ...
      if (!letters.containsKey(code)) {
        letters.put(code, new Character(element));
      }
    }
...
public char[] getCodeReplaceList() {
    ...
    if (ruleArray == null)
      return null;
    for (Object element : ruleArray) {
      rule = (TransformationRule) element;
...
private void buildRule(String str, Vector ruleList) {
    if (str.length() < 1)
      return;
    for (String element : IGNORED_KEYWORDS) {
  ...
  */
  private class TransformationRule {
    ...
    private final int takeOut, matchLength;
